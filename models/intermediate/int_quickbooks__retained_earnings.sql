...
net_income_loss as (
    ...
    select
        ...
        sum(case when account_class = 'Revenue' then period_net_change else 0 end) as revenue_net_converted_change,
        sum(case when account_class = 'Expense' then period_net_change else 0 end) as expense_net_change,
        sum(case when account_class = 'Expense' then period_net_change else 0 end) as expense_net_converted_change
